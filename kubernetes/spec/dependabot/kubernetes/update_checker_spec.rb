...
RSpec.describe Dependabot::Kubernetes::UpdateChecker do
  ...
  let(:registry_tags) { fixture("kubernetes", "registry_tags", tags_fixture_name) }
    ...
    context "given a non-numeric version" do
      ...
      context "and a digest" do
        ...
        let(:headers_response) do
          fixture("kubernetes", "registry_manifest_headers", "ubuntu_17.10.json")
        end
    ...
    context "when the latest tag points to an older version" do
      ...
      context "with a paginated response" do
        let(:pagination_headers) do
          fixture("kubernetes", "registry_pagination_headers", "next_link.json")
        end
        let(:end_pagination_headers) do
          fixture("kubernetes", "registry_pagination_headers", "no_next_link.json")
        end
        before do
          stub_request(:get, repo_url + "tags/list").
            and_return(
              status: 200,
              body: fixture("kubernetes", "registry_tags", "dotnet_page_1.json"),
          ...
          stub_request(:get, repo_url + "tags/list?last=#{last}").
            and_return(
              status: 200,
              body: fixture("kubernetes", "registry_tags", "dotnet_page_2.json"),
